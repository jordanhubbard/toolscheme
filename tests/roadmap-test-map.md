...
| Check | Script | Proves |
|---|---|---|
| Log intake | `tests/intake-check.scm` | Repeated hook invocations for one event collapse to one while preserving order; all four schemas (two Claude, Codex rollouts with their JavaScript-wrapped commands, and the hook) from pasted text, tool names merged across their spellings, cache tokens and the recorded working directory carried through |
