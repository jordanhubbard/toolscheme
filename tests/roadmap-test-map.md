...
| m2 | `process-expect`: matching output satisfies while the process still runs, an exit ends the wait distinguishably from a timeout, and output from an earlier exchange does not satisfy the next wait |
...
| Check | Script | Proves |
|---|---|---|
| Log intake | `tests/intake-check.scm` | Analysis stages that walk a whole corpus survive 20,000 calls without exhausting the stack; calls are attributed to the agent that made them; repeated hook invocations for one event collapse to one while preserving order; all four schemas (two Claude, Codex rollouts with their JavaScript-wrapped commands, and the hook) from pasted text, tool names merged across their spellings, cache tokens and the recorded working directory carried through |
...
| Analytical store | `tests/sql-check.scm` | Reports absence rather than failing when DuckDB was not built; a query cannot read outside the capability root, cannot widen the sandbox, and a bad query is a structured error |
| Continuation | `tests/continue-check.scm` | A named next step continues; a question stops; a message with both stops, because the question wins; a plain sign-off stops; the cap bounds the loop; off unless switched on |
| Advice | `tests/steer-check.scm` | A setting is read from a config file with comments, spacing and same-prefix names handled; a long wait is recognized in both spellings and advised once per session; a short one and a differing argument are passed over in silence; a repeat is advised every time; advice alone carries no permission decision |
