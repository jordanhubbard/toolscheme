...
| Advice | `tests/steer-check.scm` | A long wait is recognized in both spellings and advised once per session; a short one and a differing argument are passed over in silence; a repeat is advised every time; advice alone carries no permission decision |
